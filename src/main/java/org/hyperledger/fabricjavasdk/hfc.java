 ...
 * @returns
 */
Chain newChain(Stirng name) {
      Chain chain = _chains.get(name);
      if(chain) throw Exception(String.format("chain %s already exists", name));
	chain = new Chain(name);
	_chains.put(name, chain);
	return chain;
}

Chain getChain(String chainName, bool create) {
	Chain chain = _chains[chainName];
	if(!chain && create) chain = newChain(name);
	return chain;
}
...
class Endpoint {
    ...
    public Endpoint(String url, pem?:string) {
        let purl = parseUrl(url);
        let protocol = purl.protocol.toLowerCase();
        if (protocol === 'grpc') {
            this.addr = purl.host;
...
class MemberServicesImpl implements MemberServices {
    ...
    public crypto.Crypto getCrypto() {
        return this.cryptoPrimitives;
    }
     ...
     * @param cb Callback of the form: {function(err,{key,cert,chainKey})}
     */
    public void enroll(EnrollmentRequest req, cb:EnrollCallback) {
        let self = this;
        cb = cb || nullCB;
     ...
     * @param {function(err,[Object])} cb The callback function which is called with an error as 1st arg and an array of tcerts as 2nd arg.
     */
    public void getTCertBatch(GetTCertBatchRequest req, cb:GetTCertBatchCallback) {
        let self = this;
        cb = cb || nullCB;
     ...
     * Process a batch of tcerts after having retrieved them from the TCA.
     */
    private processTCertBatch(GetTCertBatchRequest req, resp:any):TCert[] {
        let self = this;
...
class FileKeyValStore implements KeyValStore {
    ...
    private String dir;   // root directory for the file store

    public FileKeyValStore(String dir) {
        this.dir = dir;
        if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir);
        }
     ...
     * @param cb function(err,value)
     */
    public getValue(name:string, cb:GetValueCallback) {
        var path = this.dir + '/' + name;
        fs.readFile(path, 'utf8', function (err, data) {
            if (err) {
                if (err.code !== 'ENOENT') return cb(err);
...
// Return a unique string value for the list of attributes.
String getAttrsKey(attrs?:string[]) {
    if (!attrs) return "null";
    let key = "[]";
    for (let i = 0; i < attrs.length; i++) {
       key += "," + attrs[i];
...
// Determine if an object is a string
boolean isString(obj:any) {
    return (typeof obj === 'string' || obj instanceof String);
}

// Determine if 'obj' is an object (not an array, string, or other type)
boolean isObject(obj:any) {
    return (!!obj) && (obj.constructor === Object);
}

boolean isFunction(fcn:any) {
    return (typeof fcn === 'function');
}
